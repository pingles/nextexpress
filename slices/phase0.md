...
- **In Scope**
  ...
  - `tokio` dependency with the `rt-multi-thread` and `macros` features (the runtime SLICES.md's "Concurrency model" section commits to). `app::main` is `#[tokio::main]`; the binary boots an empty runtime and exits cleanly.
...
- **Out of Scope**
  ...
  - A `Transport` trait abstraction — Telnet is the only transport for now; the abstraction lands when a second transport (SSH, FTP) does.

...
- `n` is a **buffered prefix, not a stop verb** (VERIFIED aquascan4.txt U1, mid-list AND post-End behave identically): echo `n`, hold pending — the door waits for the next byte because `n` is ambiguous between `N`(=Quit, per the in-pager help) and `ns`. The **next** key first emits `\x08 \x08` (erasing the n), then its verb runs (`n` … `\x08 \x08Quit`). Under D2's Silent *line* reads this maps naturally: line `n` → Quit (the help pins `(N),(Q) … Quit`), line `ns` → non-stop flow; D2b's true hotkeys must implement the buffering.
- `ns` → echo `n`, clear, `\x1b[36mNon-stop scrolling! Are you sure \x1b[32m(\x1b[33mY\x1b[32m/\x1b[33mn\x1b[32m)\x1b[36m? ` (Silent read); `Y` (unechoed) → clear, stream remainder non-stop (aquascan3.txt:361 + S7 repr :490); decline (`n`, unechoed) → clear, More? redrawn, paged mode continues (VERIFIED aquascan4.txt U3).
...
- `F` → clear, `\x1b[36mFile name(s) to flag:\x1b[0m ` Silent read; handler echoes the input verbatim (**no trailing CRLF** — aquascan3.txt:212-217, S4); `\r`+79sp+`\r`; redraw More?. Input **read and discarded** — flagging is silent in the captures, so this is wire-identical until D5 wires FlaggedFile.

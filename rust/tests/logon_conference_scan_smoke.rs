...
use nextexpress::app::services::SharedMailStores;
use nextexpress::domain::conference::{Conference, MessageBase, MessageBaseRef, ScanFlag};

use tokio::net::TcpStream;

use support::{
    contains, drain_until, empty_file_repo, end_session, spawn_seeded_sysop, write_line,
    TestRuntime,
};
...
/// Builds a [`TestRuntime`] with two conferences (both accessible to
/// the seeded sysop) backed by file stores at the supplied temp
/// directories, clears the `mail_scan` flag on conference 1, then binds
/// a listener via [`spawn_seeded_sysop`].
async fn spawn_two_conference_listener(
    ...
    let fixture = TestRuntime::new(bbs_path, conferences, mail_stores, empty_file_repo())
        // Clear conference 1's `mail_scan` flag (the legacy
        // `checkMailConfScan` gate / the `CF` editor's effect) so the
        // logon scan skips it while still scanning conference 2.
        .with_sysop(|sysop| {

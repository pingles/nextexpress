...
| Echoed high-byte input | The line codec keeps the echo valid UTF-8: a well-formed UTF-8 multibyte character (e.g. `é` = `\xc3\xa9`) is assembled and echoed whole and round-trips to the line; a byte that cannot be part of a valid sequence (a lone `\xa9`, a truncated lead) is re-encoded as Latin-1 (`\xa9` → `\xc2\xa9`), the inbound analogue of the outbound © rule above | Passes the raw typed byte straight back (`express.e:2335` echoes the accepted `ch>31` byte); a UTF-8 client's `é` is rendered as two Latin-1 glyphs `Ã©`, a lone `\xa9` stays `\xa9` on the wire |

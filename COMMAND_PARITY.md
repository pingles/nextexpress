...
- **Major missing surface:** `MS` listing rows (11) — planned as Slice B3; `N` rebind off mail (12) — Slice B2.
- **`MS` multi-conf walk** — planned as Slice B1 (currently `MS` scans only the current conference).
...
5. **Slices B3 / B1** (on the plan) cover the mail-scan listing and the `MS` multi-conf walk.

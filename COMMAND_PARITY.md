...
- **`MS` multi-conf walk + listing rows (11) — Done (Slices B1 + B3):** `MS` now walks every accessible conference and prints the `Type/From/Subject/Msg` table per base. Deferred to B4/B5: the `Would you like to read it now ` prompt + drop-into-read. `N` rebind off mail (12) — still Slice B2.

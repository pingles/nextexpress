...
digraph flow {
  ...
  cap [label="Stage 2\ncapture"]; des [label="Stage 3\ndesign (GATE)"];
  bld [label="Stage 4\nbuild"]; cmp [label="Stage 5\ncompare (GATE)"];
  div [label="divergence?" shape=diamond];
  res [label="Stage 6 resolve:\nask you (GATE)"]; done [label="merge to main"];
  cap -> des -> bld -> cmp -> div;
  div -> res [label="yes"]; div -> done [label="no"];
  res -> des [label="updated plan"];
}
```
...
| "The AquaScan capture is the reference, I'll match it." | For door-shadowed tokens (F/FR/N/SCAN/NS/NSU/CS/SENT) door-vs-source is **your call** — halt and ask, express.e-wins default (§10.3). |
...
2. **Ports:** `python resources/allocate_ports.py` picks a free FS-UAE host port + a free
   NextExpress port and checks nothing we'd corrupt is already running.

...
| `JM <n>` | `express.e:25185` | [cmds-conf-nav.md](slices/cmds-conf-nav.md) | C | Done (C4a — in-range `JM <n>` re-runs the full join sequence on the current conference (multi-base announcements append ` [<base>]`, no "already there" check); single-base conferences fail with the verbatim "does not contain multiple message bases" notice; a `.`-dotted first token delegates the raw params to `J`, whose dotted / two-token forms now join the requested base; missing/out-of-range bases defer to C4b's `Message Base Number (1-N): ` prompt — interim silent return, deviations recorded in the slice doc) |

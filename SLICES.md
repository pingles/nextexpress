...
| `A` list flagged set | `express.e:24601` | [cmds-files-list.md](slices/cmds-files-list.md) | D | Done (D6a — bare `A` runs the genuine internal `alterFlags` -> `showFlags`: empty prints `No file flags`, else the upper-cased flagged names space-joined, each framed by a blank line; returns to the menu. The `Filename(s) to flag:` prompt loop is D6b. Byte-pinned to `comparison/transcripts/ae_tierd_alterflags.txt`; parser + 2 handler unit tests + a telnet smoke, live-verified against both the AE reference and the Rust server) |
